...
var color 		= "#e54d06";
var bot_image 	= "https://images-ext-2.discordapp.net/external/ZnCu3NrxlBy76ah7EI5nRUvsFUTp7xY0kC54arnoNcs/https/i.imgur.com/EbZLZDf.png";
var ownerID		= "352078232188747789";
var ownerID2	= "352078232188747789";
...
bot.on('ready',async () => {
	...
	bot.user.setActivity('Slujindu-l pe Felix // SRA',{type: "playing"});
...
bot.on('message', async message => {
	...
	switch(command){
		...
		case 'info': {
			...
			.setColor(color)
			.setAuthor('Felix // SRA', bot_image, 'https://crysgfx.com/')
			.setDescription('Felix // SRA Personal bot')
			.setThumbnail(bot_image)
			.addField('Bot OWNER/Editor', 'Felix // SRA')
			.addField('Special Thanks', 'CrysADV, NILMER', true)
			.setTimestamp()
		...
		case 'invite': {
		 let user = message.mentions.users.first() || message.author;
			...
			.setAuthor('Click here for invite!', bot_image, 'https://discordapp.com/oauth2/authorize?client_id=602935115072536606&scope=bot&permissions=2146958591')
			.setColor(color)
			.setTitle(`Invite Felix // SRA to your discord server!`)
		...
		case 'choice': {
			if(!args[1] || !args[2])return message.channel.send('```Syntax: f?choice [choice1] [choice2]```');
			var choice1 = args[1];
			var choice2 = args[2];
		...
		case 'dmall': {
			...
			if(!text)return message.channel.send('```Syntax: f?dm [text]```');
		...
		case 'dm': {
            ...
            .setTitle('```Syntax: f?dm [@user] [message]```')
            .setColor(color);
		...
		case 'exec': {
			...
			let text = args.slice(1).join(' ');
			if(!text)return message.channel.send('```Syntax: f?exec [command]```');
			console.log(text);
			return;
		}
		case 'kill': {
		...
		case 'say': {
			if(message.author.id != ownerID && message.author.id != ownerID2)return;
			let text = args.slice(1).join(' ');
			if(!text)return message.channel.send('```Syntax: f?say [text]```');
			message.delete().catch(e=>{}); 
		...
		case 'addalt': {
			if(message.author.id == "352078232188747789" || message.author.id == "352078232188747789"){
				var type = args[1];
				var email = args.slice(2).join(" ");
				if(!type)return message.channel.send('```Syntax: f?addalt [type] [email:password]\n \nTypes: 1 = Spotify, 2 = Fortnite, 3 = Minecraft, 4 = NordVPN, 5 = Netflix```');
				if(!type)return message.channel.send('```Syntax: f?addalt [type] [email:password]\n \nTypes: 1 = Spotify, 2 = Fortnite, 3 = Minecraft, 4 = NordVPN, 5 = Netflix```');
				if(!type)return message.channel.send('```Syntax: f?addalt [type] [email:password]\n \nTypes: 1 = Spotify, 2 = Fortnite, 3 = Minecraft, 4 = NordVPN, 5 = Netflix```');
		...
		case 'checkaccounts': {
			if(message.author.id == "325349909412970496" || message.author.id == "325349909412970496"){
				var type = args[1];
				if(!type)return message.channel.send('```Syntax: f?checkaccounts [type]\n \nTypes: 1 = Spotify, 2 = Fortnite, 3 = Minecraft, 4 = NordVPN, 5 = Netflix```');
				if(type > 5 || type < 1)return message.channel.send('Invalid type!');
				con.connect(function(err){
		...
		case 'getalt': {
			...
			if(!type)return message.channel.send('```Syntax: f?getalt [type]\n \nTypes: 1 = Spotify, 2 = Fortnite, 3 = Minecraft, 4 = NordVPN, 5 = Netflix```');
			if(type > 5 || type < 1)return message.channel.send('Invalid type!');
			con.connect(function(err){
		...
		case 'kick': {
			if (!message.member.hasPermission("KICK_MEMBERS") && message.author.id !== ownerID && message.author.id !== ownerID2) return message.channel.send("Sorry, you don't have permissions to use this!");
			    ...
			    if(!member) return message.channel.send('```Syntax: f?kick [@user] [reason]```');
		...
		case 'ban': {
	        ...
	        let member = message.mentions.members.first();
	        if(!member) return message.channel.send('```Syntax: f?ban [@user] [reason]```');
		...
		case 'mute': {
		  ...
		  if(!tomute) return message.channel.send("```Syntax: f?mute [@user] [time(miliseconds)]\n 1 sec = 1000 miliseconds```");
		  ...
		  if(!mutetime) return message.channel.send("```Syntax: f?mute [@user] [time(miliseconds)]\n 1 sec = 1000 miliseconds```");
		...
		case 'anno': {
			...
			if(!text)return message.channel.send('```Syntax: f?anno [text]```');
			var channel = bot.channels.find(channel => channel.name === "ɴᴇᴡs");
			if(channel){
				channel.send(text);
				message.delete();
			}else {
		...
		case 'addrole': {
		  ...
		  .addField("Usage", "f?addrole [@user] [role]", true)
		  .addField("Example", "f?addrole @Felix // SRA Member")
		...
		case 'removerole': {
		    ...
		    let helpembxd = new Discord.RichEmbed()
		    .setColor(color)
		    .addField("Removerole Command", "Usage: f?removerole <@user> <role>")
		  ...
		  .addField("Usage", "f?removerole [user] [role]", true)
		  .addField("Example", "f?removerole @Felix // SRA Member")
...
Please provide a value to select one of the search results ranging from 1-10.
		...
		if (msg.author.id != ownerID && msg.author.id != ownerID2)return msg.channel.send('Doar developerul meu are access :) !');
		if (!msg.member.voiceChannel) return msg.channel.send('You are not in a voice channel!');
...
bot.on('guildMemberAdd', member => {
	var channel = bot.channels.find(channel => channel.name === "welcome");
	if(channel){
